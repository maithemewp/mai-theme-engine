...
( function( document, $, undefined ) {
	...
	if ( hasShrink ) {
		...
		var shrinkScrollDir = [];
		var firstLogoWidth  = logoWidth;
		var shrunkLogoWidth = Math.round( firstLogoWidth * .75 );
		var firstMarginTop  = parseInt( $logoLink.css( 'marginTop' ) );
		var firstMarginBot  = parseInt( $logoLink.css( 'marginBottom' ) );
		...
		.on( 'progress', function(e) {
			...
			var currentLogoWidth = $logoLink.outerWidth();
			var currentMarginTop = parseInt( $logoLink.css( 'marginTop' ) );
			var currentMarginBot = parseInt( $logoLink.css( 'marginBottom' ) );
			var newLogoWidth     = Math.round( logoWidth - ( ( logoWidth - shrunkLogoWidth ) * e.progress ) );
			var newMarginTop     = Math.round( ( .2 + ( .8 * ( 1 - e.progress ) ) ) * firstMarginTop );
			var newMarginBot     = Math.round( ( .2 + ( .8 * ( 1 - e.progress ) ) ) * firstMarginBot );
			var doLogoWidth      = shouldAdjust( e.scrollDirection, currentLogoWidth, newLogoWidth );
			var doMarginTop      = shouldAdjust( e.scrollDirection, currentMarginTop, newMarginTop );
			var doMarginBot      = shouldAdjust( e.scrollDirection, currentMarginBot, newMarginBot );
